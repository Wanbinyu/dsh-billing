...
type Family = 'flash' | 'pro'
type Era = 'pre-20260816' | '20260816' | '20260910'
type Window = 'flat' | 'peak' | 'off'

const price = (input: number, output: number, cacheRead: number): BillingModelPrice => ({
  input,
  output,
  cacheRead,
  cacheWrite: input,
...
const CARDS: Record<Family, Record<Era, Record<Window, BillingModelPrice>>> = {
  flash: {
...
export const chinaDateKey = (timeMs: number): string => {
...
export const isDeepSeekPeak = (timeMs: number): boolean => {
  const instant = new Date(timeMs)
  const weekday = instant.getUTCDay()
  if (weekday === 0 || weekday === 6) return false
  if (CN_PUBLIC_HOLIDAYS_2026.has(chinaDateKey(timeMs))) return false
  const minutes = instant.getUTCHours() * 60 + instant.getUTCMinutes()
  const inside = (startHour: number, endHour: number): boolean =>
    minutes >= startHour * 60 && minutes < endHour * 60
  return inside(1, 4) || inside(6, 10)
}

const familyOf = (model: string): Family | undefined =>
  FLASH_IDS.has(model) ? 'flash' : PRO_IDS.has(model) ? 'pro' : undefined

const eraOf = (timeMs: number): Era =>
  timeMs < AUG16_UTC ? 'pre-20260816' : timeMs < SEP10_UTC ? '20260816' : '20260910'
 ...
 */
export const officialSliceKey = (provider: string, model: string, timeMs: number): string | null => {
  if (!FIRST_PARTY.has(provider)) return null
  const family = familyOf(model)
  if (family === undefined) return null
  const era = eraOf(timeMs)
  const window: Window = era === 'pre-20260816' ? 'flat' : isDeepSeekPeak(timeMs) ? 'peak' : 'off'
  return `${family}:${era}:${window}`
}

/** USD card named by {@link officialSliceKey}, or undefined for a foreign key. */
export const priceForSliceKey = (sliceKey: string): BillingModelPrice | undefined => {
  const [family, era, window] = sliceKey.split(':')
  if (family !== 'flash' && family !== 'pro') return undefined
  if (era !== 'pre-20260816' && era !== '20260816' && era !== '20260910') return undefined
  if (window !== 'flat' && window !== 'peak' && window !== 'off') return undefined
